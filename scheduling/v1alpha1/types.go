...
// +kubebuilder:resource:shortName=cnrp
...
// +kubebuilder:object:root=true
// +kubebuilder:subresource:status
// +kubebuilder:resource:shortName=nrp
...
type ScheduleApplyScope struct {
	...
	// Selectors is applied scoped which means all newly pods satisfied the LabelSelector & Namespace will use newly predicates & priorities to do scheduling
	// key is namespace, value is label selector
	// 1. if there are no namespaces, then all newly pods will keep the original scheduling logic
	// 2. if there has some namespaces, but namespaces has no LabelSelector(is null selector), then all pods in these namespaces are applied with newly scheduling policy
	// 3. if there has some namespaces, and each namespace has its valid label selector, then all pods satisfy the namespace & label selector will be applied with newly scheduling policy
	// +optional
	Selectors map[string] /*namespace*/ metav1.LabelSelector `json:"selectors,omitempty"`
}
